...
function ColumnDropZone({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`space-y-2 min-h-[120px] rounded-md transition-colors ${
        isOver ? 'bg-accent/40' : ''
      }`}
    >
      {children}
...
export function DraggableKanban({ tasks, onTaskStatusChange, onTaskDelete }: Props) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const newStatus: Task['status'] | undefined = (['todo', 'inprogress', 'done'] as const).includes(overId as any)
      ? (overId as Task['status'])
      : getTaskStatus(overId)
